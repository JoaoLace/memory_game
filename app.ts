class Card {
  ...
  positionY: number;
  isFlipped: boolean;
  gotItCorrect: boolean;
  constructor(id: number, img: string, positionX: number, positionY: number) {
    this.id = id;
...
function createCardHTML(card: Card) {
...
function testWin() {
  var won = true;
  Cards.forEach((element) => {
    if (!element.gotItCorrect) won = false;
  });
